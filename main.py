...
from utils import _build_model, _ensure_2d, _plot_series, build_loader, forecast_full_trajectory, load_data, plot_test_series_prediction, split_train_val
...
def eval_pinn(config: Dict) -> Dict:
    ...
    for idx, sample in enumerate(test_series):
        ...
        target = sample.get("y") or sample.get("u")
        if target is None:
            raise ValueError("Test sample must contain 'y' (states over time).")
...
if __name__ == "__main__":
    ...
    with open("config/eval_PINN.yaml", "r") as fh:
        cfg = yaml.safe_load(fh)
